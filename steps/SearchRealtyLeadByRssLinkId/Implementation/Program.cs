    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"Searching for realty leads with RSS Link ID: {rssLinkId}");
        ...
        {
            Console.WriteLine("SearchRealtyLeadByRssLinkId - Search for realty leads by RSS Link ID");
            Console.WriteLine();
            Console.WriteLine("Usage:");
            Console.WriteLine("  SearchRealtyLeadByRssLinkId.exe --env <environment> --rsslinkid <guid>");
            Console.WriteLine();
            ...
            Console.WriteLine("  SearchRealtyLeadByRssLinkId.exe --env UAT --rsslinkid 12345678-1234-1234-1234-123456789abc");
            Console.WriteLine("  SearchRealtyLeadByRssLinkId.exe --env PROD --rsslinkid 87654321-4321-4321-4321-cba987654321");
            Console.WriteLine("  SearchRealtyLeadByRssLinkId.exe --env UAT --rsslinkid 12345678-1234-1234-1234-123456789abc --output-format json");
        }
        ...
        private static List<LeadSearchResult> SearchLeadsByRssLinkIdMock(Guid rssLinkId, bool verbose = true)
        {
            ...
            for (int i = 0; i < leadCount; i++)
            {
                ...
                {
                    LeadId = Guid.NewGuid(),
                    FirstName = $"MockRealtyFirst{i + 1}",
                    LastName = $"MockRealtyLast{i + 1}",
                    Email = $"realtymock{i + 1}@example.com",
            ...
            try
            {
                ...
                foreach (var entity in response.Entities)
                {
                    var result = new LeadSearchResult
                    {
                        ...
                        Phone = entity.GetAttributeValue<string>("rss_mostrecentphonenumber") ?? 
                               entity.GetAttributeValue<string>("telephone1") ?? 
                               entity.GetAttributeValue<string>("telephone2") ?? 
                               entity.GetAttributeValue<string>("mobilephone") ?? string.Empty,
                        RssLinkId = Guid.Parse(entity.GetAttributeValue<string>("rss_rsslinkid") ?? Guid.Empty.ToString()),
